...
namespace sf
{
    ...
    struct Shader::UniformBinder : private NonCopyable
    {
            ...
            location(-1)
        {
            ...
            {
                // Enable program object
               glCheck(GLEXT_glGetIntegerv(GLEXT_GL_CURRENT_PROGRAM, (GLint *)&savedProgram));
                if (currentProgram != savedProgram)
                    glCheck(GLEXT_glUseProgram(currentProgram));
        ...
        ~UniformBinder()
        {
            // Disable program object
            if (currentProgram && (currentProgram != savedProgram))
                glCheck(GLEXT_glUseProgram(savedProgram));
        }
    ...
    void Shader::bind(const Shader* shader)
    {
        ...
        {
            // Enable the program
            glCheck(GLEXT_glUseProgram(castToGlHandle(shader->m_shaderProgram)));
    ...
    bool Shader::compile(const char* vertexShaderCode, const char* geometryShaderCode, const char* fragmentShaderCode)
    {
        ...
        if (m_shaderProgram)
        {
            glCheck(GLEXT_glDeleteShader(castToGlHandle(m_shaderProgram)));
            m_shaderProgram = 0;
        }
        ...
        // Create the program
        GLEXT_GLhandle shaderProgram;
        glCheck(shaderProgram = GLEXT_glCreateProgram());
        ...
        if (vertexShaderCode)
        {
            // Create and compile the shader
            GLEXT_GLhandle vertexShader;
            glCheck(vertexShader = GLEXT_glCreateShader(GLEXT_GL_VERTEX_SHADER));
            glCheck(GLEXT_glShaderSource(vertexShader, 1, &vertexShaderCode, NULL));
            glCheck(GLEXT_glCompileShader(vertexShader));

            // Check the compile log
            GLint success;
            glCheck(GLEXT_glGetShaderiv(vertexShader, GLEXT_GL_COMPILE_STATUS, &success));
            if (success == GL_FALSE)
            {
                char log[1024];
        ...
        {
            // Create and compile the shader
            GLEXT_GLhandle geometryShader = GLEXT_glCreateShader(GLEXT_GL_GEOMETRY_SHADER);
            glCheck(GLEXT_glShaderSource(geometryShader, 1, &geometryShaderCode, NULL));
            glCheck(GLEXT_glCompileShader(geometryShader));

            // Check the compile log
            GLint success;
            glCheck(GLEXT_glGetShaderiv(geometryShader, GLEXT_GL_COMPILE_STATUS, &success));
            if (success == GL_FALSE)
            {
                char log[1024];
        ...
        if (fragmentShaderCode)
        {
            // Create and compile the shader
            GLEXT_GLhandle fragmentShader;
            glCheck(fragmentShader = GLEXT_glCreateShader(GLEXT_GL_FRAGMENT_SHADER));
            glCheck(GLEXT_glShaderSource(fragmentShader, 1, &fragmentShaderCode, NULL));
            glCheck(GLEXT_glCompileShader(fragmentShader));

            // Check the compile log
            GLint success;
            glCheck(GLEXT_glGetShaderiv(fragmentShader, GLEXT_GL_COMPILE_STATUS, &success));
            if (success == GL_FALSE)
            {
                char log[1024];
        ...
        // Check the link log
        GLint success;
        glCheck(GLEXT_glGetProgramiv(shaderProgram, GLEXT_GL_LINK_STATUS, &success));
        if (success == GL_FALSE)
        {
            char log[1024];
